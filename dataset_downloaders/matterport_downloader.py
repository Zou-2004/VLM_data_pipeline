...
def main():
    ...
    parser.add_argument('-o', '--out_dir', '--output-dir', required=True, help='directory in which to download')
    ...
    parser.add_argument('--auto-accept', action='store_true', help='Auto-accept terms of service (use with caution)')
    args = parser.parse_args()

    # Auto-accept TOS when --auto-accept flag is provided
    if hasattr(args, 'auto_accept') and args.auto_accept:
        print('Auto-accepting Matterport TOS as described at:')
        print(TOS_URL)
    else:
    ...
    if args.task_data:
        ...
        if not (hasattr(args, 'auto_accept') and args.auto_accept):
    ...
    if args.type:
        ...
        if invalid_types:
            print('ERROR: Invalid file type: ' + ', '.join(invalid_types))
            return
